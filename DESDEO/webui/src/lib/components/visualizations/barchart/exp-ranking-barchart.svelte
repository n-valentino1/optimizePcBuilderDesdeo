	...
	function drawHorizontalChart(
		...
		svgElement
			...
			.each(function () {
				...
				const maxChars = Math.floor(margin.left / 6);
				if (fullText.length > maxChars) {
					text.text(fullText.substring(0, maxChars - 1) + '…');
				}
			});
	}

	function drawChart(): void {
		let margin = { top: 0, right: 30, bottom: 0, left: 30 };

		// Adapt left margin based on longest objective name, capped for sidebar
		const longestNameLength = d3.max(data, (d) => d.name.length) ?? 0;
		const maxLeftMargin = Math.min(width * 0.4, 120);
		margin.left = Math.min(Math.max(10, longestNameLength * 5), maxLeftMargin);
...
<div bind:this={container} style="min-height: {Math.max(100, (data.length - 1) * 35)}px; width: 100%;">
