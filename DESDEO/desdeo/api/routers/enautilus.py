...
def step(
    ...
    if request.current_iteration == 0:
        ...
        # Some representative sets only store the minimized columns (e.g. f_1_min),
        # while older data may include both original and minimized values.
        solution_data = representative_solutions.solution_data
        first_objective_symbol = problem.objectives[0].symbol
        first_objective_key = (
            first_objective_symbol if first_objective_symbol in solution_data else f"{first_objective_symbol}_min"
        )

        selected_point = {
            f"{obj.symbol}": (-1 if obj.maximize else 1)
            * np.max(solution_data.get(f"{obj.symbol}_min", solution_data.get(obj.symbol, [0.0])))
            for obj in problem.objectives
        }
        reachable_point_indices = list(range(len(solution_data[first_objective_key])))
    else:
        # Not first iteration
