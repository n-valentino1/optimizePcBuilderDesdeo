...
def get_current_user(
    ...
    Args:
        ...
        cookie_token (Annotated[str | None, Security(cookie_scheme)]): The authentication token as part of request cookie.
    ...
    """
    # No-auth / development mode: return the guest user when DESDEO_NO_AUTH is set
    try:
        no_auth = os.getenv("DESDEO_NO_AUTH", "").lower() in ("1", "true", "yes")
    except Exception:
        no_auth = False

    if SettingsConfig.debug and no_auth:
        statement = select(User).where(User.username == "guest")
        guest = session.exec(statement).first()
        if guest is None:
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="No guest user found for no-auth mode")
        return guest
